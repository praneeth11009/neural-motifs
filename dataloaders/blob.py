...
class Blob(object):
    ...
    def _scatter(self, x, chunk_sizes, dim=0):
        """ Helper function"""
        if self.num_gpus == 1:
            return x.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True
    ...
    def scatter(self):
        ...
        self.gt_classes_primary = self.gt_classes.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True
        self.gt_boxes_primary = self.gt_boxes.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True
        ...
        if self.is_train:
            ...
            self.train_anchor_inds = self._scatter(self.train_anchor_inds,
                                                   self.train_chunks)
            self.train_anchor_labels = self.train_anchor_labels.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True
            self.train_anchors = self.train_anchors.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True

            if self.is_rel:
                self.gt_rels = self._scatter(self.gt_rels, self.gt_rel_chunks)
        else:
            if self.is_rel:
                self.gt_rels = self.gt_rels.cuda(self.primary_gpu, non_blocking=True) # use this for pytorch0.4: non_blocking=True
